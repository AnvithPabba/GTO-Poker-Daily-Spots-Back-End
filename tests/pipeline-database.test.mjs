...
test("validated pipeline persists split payloads idempotently and rejects conflicting versions", { skip: !databaseUrl && "DATABASE_URL is not set" }, async () => {
  ...
  const publicPayload = {
    schemaVersion: 1, spotId: ids.spot, spotVersionId: ids.version, mode: "single_hand", publicationDate: "2026-08-21", slotOrder: 1,
  ...
  const envelope = { schemaVersion: 1, sourceHash, publicPayload, privateSolutionPayload: privatePayload, candidateManifest: { sourceHash, path: ["root", "decision"], selectedCombo: "AhAs", fallbackUsed: false, rankingVersion: "1" }, provenance: { normalizerVersion: "1", selectionRankingVersion: "1" } };
